...
対象を指定するアクションでは、`key`と`finder`のどちらか一方だけを指定します。Finderは画面上の対象を一つだけ見つける必要があります。アクションIDはパッケージが生成します。カスタムコントロールの追加の利用可能条件には`availableWhen: () => ...`を使えます。ピンチ操作は標準では提供していません。対応する対象、時間設定、サンプルは[アクションリファレンス](docs/actions.md)を参照してください。
...
ローカルのテストはパッケージの動作を検証するもので、実際のモデル判断の精度を実証するものではありません。詳しくは[アクションリファレンス](docs/actions.md)と[変更履歴](CHANGELOG.md)を参照してください。
